...
|  | **UX기획자** (UX owner) | 화면 목록·와이어프레임·상태 4종(정상/빈/에러/로딩)·접근성. M16 — Screen ID `UI-{명칭}-{NN}` BN 표준 (S-NNN 폐기) |
...
- 에이전트 자가 점검 통과율이 85% 미만이면 *완성 산출물로 사용 거부*한다 (§4-0 (iv)·§6 참조 — 후행 진입 거부 아님)
...
| **B — 산출물 게이트** | 산출물 완성 후, *완성 산출물 사용 전* | 자동 | **advisory** | 결함 전파 차단 (PRD 결함 → 기술검토·UX·퍼블까지 오염). 단 부분 broadcast/reply 흐름은 게이트 무관 (§4-0 (ii)·(iv)) |
...
독립 조사 작업(시장 분석·경쟁사 조사 등) / UI 무관 영역(퍼블 `assets/{tokens, css, js}/`은 M9 §2-5 정합 UI broadcast 무관 선행 가능 — M16: UX 영역 ID `UI-{명칭}-{NN}` 정합)
...
└── <slug>/
    ...
    ├── 01-prd.md                   # 서비스기획자 — 6 섹션 (§0 PM 원본 + §A 비전 + §B 16 필드 카탈로그 + §C 표준 패턴 + §D 오픈 이슈 + §E 자가 점검)
...
├── M{N}_*.md                       # M-시리즈 design doc / plan / session_resume
└── _codex-gate-*-output.log        # Codex Gate A·B·C 라운드별 보존
...
**M16 갱신 (PI-003·PI-004·PI-019)** — TR만 영역별 독립 시퀀스 보존, REQ 4 segment 갱신, UX·P 영역 BN 표준 단일 사용:

| 영역 | M16 ID 형식 | 정규식 | 비고 |
|------|-------------|--------|------|
| REQ (서비스기획자) | `REQ-{도메인 2~4글자}-{NNN}-{NN}` | `^REQ-[A-Z]{2,4}-\d{3}-\d{2}$` | 도메인 약어 + 시퀀스 3자리 + 상세 2자리 (PI-004) |
| TR (기술검토자) | `TR-NNN` | `^TR-\d{3}$` | 영역별 독립 시퀀스 보존 (M10 정합) |
| UX (UX기획자) | `UI-{명칭}-{NN}` | `^UI-[A-Za-z][A-Za-z0-9_]*-\d{2}$` | BN 표준 단일 사용 (PI-019) — S-NNN 폐기 |
| P (퍼블리셔) | `UI-{명칭}-{NN}` (UX와 동일) | (위와 동일) | UX와 동일 ID 사용 (PI-019 A1) — P-NNN 폐기 |
| 기능명세서 | `FN-NNN` | `^FN-\d{3}$` | 옵션 산출물 신설 시 (PI-005) |

**불변식 I14 (M10 정합 + M16 갱신)** — ID 영역별 발급 규칙:

| 영역 | 발급 규칙 | rename | 폐기·결번 |
|------|----------|--------|-----------|
| REQ (`REQ-{도메인}-NNN-NN`) | *도메인별 독립 시퀀스*: 도메인 내 NNN = `max(NNN)+1` (도메인 간 무관). NN 상세번호 = NNN별 독립 시퀀스 (`REQ-USR-001-01` → `REQ-USR-001-02`는 같은 NNN의 상세 분리. 다른 NNN은 `REQ-USR-002-01`) | 도메인 변경(USR→ADM) 시 = 폐기 + 신규 ID 발급 (rename X) | 결번 허용 (폐기·오발급 교정 시 Decision Log 기록) |
| TR (`TR-NNN`) | 영역별 독립 시퀀스 `max(NNN)+1` | rename X | 결번 허용 |
| UX·P (`UI-{명칭}-{NN}`) | *명칭별 독립 시퀀스*: 명칭 내 NN = `max(NN)+1` (명칭 간 무관. 예: `UI-Login-01`·`UI-Login-02` 같은 명칭의 변형 / `UI-Member_List-01`은 별도 명칭) | 화면명 변경 시 = 폐기 + 신규 ID 발급 (rename X — 추적성 보존) | 결번 허용 (폐기 시 Decision Log 기록 + README NA list 등재) |
| FN (`FN-NNN`) | 옵션 산출물 신설 시. TR과 동일 규칙 | rename X | 결번 허용 |

**공통 불변식**:
- ID 변경 이벤트 전 과정 불변. suffix `[vN]`은 표시 레이어 (ID 구성 요소 아님)
- 폐기·재사용 금지 (영역 침범 = 같은 ID 재할당 X)
- rename 발생 시 = *폐기 + 신규 발급* 패턴 (M9 §1-2-2 정합 — 추적성 보존)

**M:N 매핑** — 항목 헤더 참조 주석 형식:
- TR: `## TR-NNN (→ REQ-{도메인}-NNN-NN, REQ-{도메인}-NNN-NN)`
- UX: `## UI-{명칭}-{NN} (→ REQ-{도메인}-NNN-NN, REQ-{도메인}-NNN-NN)`
- P (HTML): `<!-- UI-{명칭}-{NN} / → REQ-{도메인}-NNN-NN, REQ-{도메인}-NNN-NN -->`

**파일명 정책 (M16 갱신, M9 §1-2-2)**:
- 페이즈 1 default 4종: 파일명 ID 미포함 (예: `01-prd.md`, `02-tech-review.md`)
- **UX·P 영역 — 파일명 = UI ID 허용** (PI-019 A1 갱신): `pages/<UI-{명칭}-{NN}>.html` (1 화면 = 1 HTML 파일)
- 재사용 금지·결번 허용 정신은 보존 (UI ID 자체가 BN 표준이므로 충돌 없음)
...
3. **자가 점검 통과율 85% 미만 시 *완성 산출물 사용 거부*** (§4-0 (iv) 정합 — *후행 진입* 거부 아님) — 팀장 Claude가 *완성 산출물 사용 게이트* 강제. 미달 시 *완성 산출물 사용 거부* + 선행 영역 owner에게 reply (M11 자가 점검 재발동). **단 부분 broadcast/reply 흐름은 게이트 무관** — 4 Teammate는 [2] spawn 시점부터 자기 영역 진행
...
5. **자율 결정 영역 분류 (M16 신설 — PI-010·PI-011·PI-012·PI-013)**:

   PRD 명시 vs 명시 없음 처리 분류:

   | 영역 | 처리 | 카탈로그 (v1 잠정) |
   |------|------|--------------------|
   | **PRD §B 명시 사항** | 후행 결정 근거 (자율 변경 X — 영역 침범) | 16 필드 카탈로그 모든 항목 |
   | **표준 패턴 영역** (자율 결정 허용 + §C 1줄 기록) | 자율 결정 OK + service-planner가 PRD §C에 1줄 형식 기록 (`(YYYY-MM-DD) 영역: default 결정`, ID 부여 없음) | 로그인·회원가입·비밀번호 reset·세션 관리·기본 CRUD UI·이메일 발송 |
   | **경계 사례 영역** (PRD 명시 의무 + 자율 결정 금지) | service-planner가 PRD §B 16 필드(*제약사항 및 전제조건* 또는 별도 요구사항)로 명시. 후행이 미명시 발견 시 reply 의무 | SSO (Google·Apple·Naver·Kakao 등) / 결제 (PG사·결제 수단·정기 결제) / 2FA (SMS·OTP·인증 앱) / 이미지 업로드 (저장소·CDN·리사이징) |
   | **특정 로직 영역** (PM 추가 질의) | service-planner가 PRD 작성 중 발견 → controller 경유 PM `AskUserQuestion` 또는 즉시 reply → PM 결정 → §B 명시 + Decision Log | 결제 흐름 / 도메인 로직 (랭킹 산정·매칭 알고리즘·점수 계산) / 비표준 비즈니스 로직 |

   **처리 결정 트리** (후행 영역 작업 중 발견 시):
   ```
   §B 명시 사항       → 후행 결정 근거. 자율 변경 X
   §B 미명시 + 표준 패턴 → 자율 결정 OK + service-planner reply로 §C 기록 요청 (후행 직접 §C 수정 X)
   §B 미명시 + 경계 사례 → reply 의무 (자율 결정 금지)
   §B 미명시 + 특정 로직 → PM 추가 질의 (controller 경유)
   §B 미명시 + 카탈로그 외 → 안전 default = reply
   ```

   카탈로그 정련은 운영 시행착오 후 정련 (`_FOLLOWUP.md` ③).

6. **블로커 발생 시**: 작업 중단 → 팀장에게 이유·영향·옵션 3가지 보고 (§10 참조)

7. **도구 인지 사전 검증**: 운영 작업 시작 전, 사용 가능한 Skills·Agent 도구·환경 변수(예: Agent Teams 활성화 여부)를 1차 점검. 도구 활용 미숙으로 헌법 정의를 워터폴화·다운그레이드하는 것은 §10 위반.
...
| 2 | `timestamp` | 시점 |
| 3 | `owner` | 영역 enum (M16 갱신): `REQ` / `TR` / `UX` / `P` (S→UX, M16 PI-019 정합 — 한글명 `서비스기획자`·`기술검토자`·`UX기획자`·`퍼블리셔` 사용 금지, owner enum 단일화) |
...
- ❌ **PRD = 비전 문서 형식 작성 금지** (M9·M13 7 섹션 폐기) — *요구사항 정의서* 본질 (PI-001). 본문 = 6 섹션(§0~§E) + 16 필드 카탈로그
- ❌ **KPI(성공 지표) 작성 금지** (PI-008) — PRD 본질 X (측정 목표 ≠ 요구사항)
- ❌ **REQ ID 3자리 형식 사용 금지** (PI-004) — `REQ-{도메인 2~4글자}-{NNN}-{NN}` 4 segment 강제
- ❌ **Must 기능 단위 평가 금지** (M9·M13 폐기, PI-001·PI-007) — *요구사항 ID 단위* 1:1 매핑 평가만
- ❌ **섹션 단위 인용 금지** (PI-003·PI-004) — `01-prd.md §B-N REQ-{도메인}-NNN-NN` 4 segment 단위 인용 강제
- ❌ **경계 사례(SSO·결제·2FA·이미지 업로드) PRD 미명시 금지** (PI-011 — 격차 4 회귀)
...
**Mesh 본질 위배 (M15 신설)**:
- ❌ **묶음 broadcast 금지** — 부분 확정 사건마다 즉시 발행 (§4-0 (ii), 격차 2 회귀 방지)
- ❌ **자율 결정 우회 금지** — 모순·누락 발견 시 reply 의무 (§4-0 (iii), 격차 4 회귀 방지)
