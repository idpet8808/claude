...
> **PM 결정**: 본 MVP는 **S1 메인 페이지(랭킹 위젯) 1개만 우선 구현**. S2~S8은 후속 작업으로 미룸 (M13 v1 C6 운영 검증 — 가속 시연).
> 03-ux-spec.md S1 명세 그대로 매핑 (퍼블리셔 화면 가감 권한 없음 — M13-① 결정 #3).
...
| 후속 P-NNN | 화면명 | 매핑 Must / 비고 |
|-----------|--------|-----------------|
| P-002 (예정) | S2. 로그인 | F-5 (기존 멘사 DB 연동) — TR-01 미팅 결과(2026-05-09)에 따라 시나리오 A/B/C 분기 |
| P-003 (예정) | S3. 챌린지 목록 | F-2 (챌린지 무료 도전) |
| P-004 (예정) | S4-A~E. 챌린지 풀이 (5종) | F-2 + F-3 — `_reference/` 5종 콘텐츠 활용 (11배수 스도쿠 / 너는 바보입니다 / 배수 수열 / 아 치킨먹고싶다 / 아름다운 수열). **11배수 스도쿠·아름다운 수열은 전용 인터랙티브 React 컴포넌트** 별도 개발 필요 (TR 분석 결과) |
| P-005 (예정) | S5. 결과·랭킹 반영 | F-1 + F-2 |
| P-006 (예정) | S6. 힌트·패스 구매 모달 | F-3 (힌트·패스 유료 판매) |
| P-007 (예정) | S7. 유료결제 패스 상품 | F-4 — TR-04(구독형 vs 소진형) feature flag 처리 |
| P-008 (예정) | S8. 관리자 대시보드 | Should F-6 — Must 격상 PM 결정 대기 (오픈 이슈 #3) |
